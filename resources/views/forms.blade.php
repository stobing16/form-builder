    ...
    <div class="w-100 h-100 background">
        ...
        <div class="custom-container py-5 d-flex justify-content-center">
            <div class="before-card">
                ...
                <img src="{{ asset('img/links.png') }}" alt="Linksindo Logo" class="img-fluid logo mb-4" />
                                    ...
                                    @switch($question->question_type_id)
                                        @case(1)
                                            <input type="{{ $question->type->type }}" name="{{ $form->unique_url . '[' . $question->id . ']' }}" class="form-control form-control-lg" placeholder="{{ $question->question }}"
                                                data-question-id="{{ $question->id }}" data-required="{{ $question->is_required ? 'true' : 'false' }}" @if ($question->is_required) required @endif>
                                        @break

                                        @case(2)
                                            <textarea data-question-id="{{ $question->id }}" data-required="{{ $question->is_required ? 'true' : 'false' }}" class="form-control form-control-lg" placeholder="{{ $question->question }}"
                                                name="{{ $form->unique_url . '[' . $question->id . ']' }}" rows="5" @if ($question->is_required) required @endif></textarea>
                                        @break

                                        @case(3)
                                            @foreach (json_decode($question->options, true) as $option)
                                                <div class="form-check form-check-inline mt-2">
                                                    <input class="form-check-input" type="{{ $question->type->type }}" name="{{ $form->unique_url . '[' . $question->id . ']' }}" value="{{ $option }}" data-question-id="{{ $question->id }}"
                                                        data-required="{{ $question->is_required ? 'true' : 'false' }}" @if ($question->is_required) required @endif>
                                        ...
                                        @case(4)
                                            @foreach (json_decode($question->options, true) as $option)
                                                <div class="form-check mt-2 form-check-inline">
                                                    <input data-question-id="{{ $question->id }}" class="form-check-input" type="{{ $question->type->type }}" name="{{ $form->unique_url . '[' . $question->id . '][]' }}" value="{{ $option }}"
                                                        id="flexCheckDefault">
                                            ...
                                            @if ($question->has_additional_question)
                                                <div class="form-check mt-2 form-check-inline">
                                                    <input class="form-check-input check-others" id="other-{{ $question->id }}" type="{{ $question->type->type }}" data-id="{{ $question->id }}" name="other-value-{{ $form->unique_url }}"
                                                        value="">
                                                    <label class="form-check-label" for="flexCheckDefault">
                                                        Lainnya ...
                                                    </label>
                                        ...
                                        @case(5)
                                            <select class="form-select" name="{{ $form->unique_url . '[' . $question->id . ']' }}" @if ($question->is_required) required @endif data-question-id="{{ $question->id }}"
...
@push('script')
    <script>
        $(document).ready(function() {
            let formUniqueUrl = @json($form->unique_url);
            $('#responses').validate({
                rules: {
                    @foreach ($questions as $question)
                ...
                errorPlacement: function(error, element) {
                    ...
                    console.log("Error Placement Triggered");
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.fire({
                                ...
                                window.location.href = "{{ route('forms.user.success-unique', ':unique_url') }}".replace(':unique_url', formUniqueUrl);
                            });
                        },
